...
This directory contains the hosted connector only. The pure `ascendant` package
remains responsible for chart calculation and person-record artifacts.
...
settings: `requirements.txt` installs the pure library from `..`, and
`vercel.json` includes the canonical `../plugins/agent` skill package.
