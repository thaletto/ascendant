...
def Matsya(yoga: Yoga) -> YogaType:
    ...
    if not (len(malefics_in_1) > 0 and len(malefics_in_9) > 0):
        result["details"] = f"Malefics not found in both 1st ({', '.join(malefics_in_1) or 'None'}) and 9th ({', '.join(malefics_in_9) or 'None'}) houses."
        return result
    ...
    if not (len(malefics_in_5) > 0 and len(benefics_in_5) > 0):
        result["details"] = f"Both malefics ({', '.join(malefics_in_5) or 'None'}) and benefics ({', '.join(benefics_in_5) or 'None'}) not found in 5th house."
        return result
    ...
    if not (len(malefics_in_4) > 0 and len(malefics_in_8) > 0):
        result["details"] = f"Malefics not found in both 4th ({', '.join(malefics_in_4) or 'None'}) and 8th ({', '.join(malefics_in_8) or 'None'}) houses."
        return result
...
def Kurma(yoga: Yoga) -> YogaType:
    ...
    benefics_in_5 = [p for p in yoga.planets_in_relative_house("Lagna", 5) if p['name'] in BENEFIC_PLANETS]
    benefics_in_6 = [p for p in yoga.planets_in_relative_house("Lagna", 6) if p['name'] in BENEFIC_PLANETS]
    benefics_in_7 = [p for p in yoga.planets_in_relative_house("Lagna", 7) if p['name'] in BENEFIC_PLANETS]
                ...
                for planet in house_data['planets']:
                    if planet['name'] == benefic_name:
                        ...
                        if benefic_name == 'Mercury' and sign in ['Gemini', 'Virgo']:
                            is_dignified = True
                        elif benefic_name == 'Jupiter' and sign in ['Sagittarius', 'Pisces', 'Cancer']:
                            is_dignified = True
                        elif benefic_name == 'Venus' and sign in ['Taurus', 'Libra', 'Pisces']:
                            is_dignified = True
                        
                        if is_dignified:
                            dignified_benefics += 1
                        dignity_details.append(f"{benefic_name} in D9 sign {sign} (dignified: {is_dignified})")

        if dignified_benefics == len(all_benefics_names_A):
            result["present"] = True
            result["strength"] = 1.0
            result["details"] = "Benefics in 5th, 6th, 7th are all dignified in Navamsa.\n" + ", ".join(dignity_details)
            return result

    # Condition B: Benefics in 1, 3, 11 and dignified in D1
    benefics_in_1 = [p for p in yoga.planets_in_relative_house("Lagna", 1) if p['name'] in BENEFIC_PLANETS]
    benefics_in_3 = [p for p in yoga.planets_in_relative_house("Lagna", 3) if p['name'] in BENEFIC_PLANETS]
    benefics_in_11 = [p for p in yoga.planets_in_relative_house("Lagna", 11) if p['name'] in BENEFIC_PLANETS]
    ...
    if len(benefics_in_1) > 0 and len(benefics_in_3) > 0 and len(benefics_in_11) > 0:
        ...
        for p in all_benefics_B:
            ...
            if is_dignified:
                 dignified_count += 1
            dignity_details_B.append(f"{p['name']} in D1 sign (dignified: {is_dignified})")

        if dignified_count == len(all_benefics_B):
            result["present"] = True
            result["strength"] = 1.0
            result["details"] = "Benefics in 1st, 3rd, 11th are all dignified in Rasi.\n" + ", ".join(dignity_details_B)
            return result
...
def Devendra(yoga: Yoga) -> YogaType:
    ...
    if not (house_of_lord_1 == 11 and house_of_lord_11 == 1):
        result["details"] = f"Lords of 1st ({lord_of_1} in {house_of_lord_1}) and 11th ({lord_of_11} in {house_of_lord_11}) do not interchange houses."
        return result
    ...
    if not (house_of_lord_2 == 10 and house_of_lord_10 == 2):
        result["details"] = f"Lords of 2nd ({lord_of_2} in {house_of_lord_2}) and 10th ({lord_of_10} in {house_of_lord_10}) do not interchange houses."
        return result
...
def Makuta(yoga: Yoga) -> YogaType:
    ...
    if relative_pos_ju != 9:
        result["details"] = f"Jupiter is in {relative_pos_ju} from 9th lord ({lord_of_9}), not 9th."
        return result

    # Condition 2: A benefic in 9th from Jupiter
    planets_9th_from_ju = yoga.planets_in_relative_house("Jupiter", 9)
    benefics_9th_from_ju = [p['name'] for p in planets_9th_from_ju if p['name'] in BENEFIC_PLANETS]
...
def Chandika(yoga: Yoga) -> YogaType:
    ...
    if not (house_nsl6 == house_sun and house_nsl9 == house_sun):
        result["details"] = f"Navamsa lords of L6({NSL6} in {house_nsl6}) and L9({NSL9} in {house_nsl9}) are not co-joined with Sun (in {house_sun})."
        return result
