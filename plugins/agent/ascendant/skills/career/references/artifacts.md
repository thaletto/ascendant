...
Legacy `parashari_raman_v1` provenance describes the method active when the
record was saved; its chart calculations remain usable by the current v2
hierarchy. Re-running `init-person` with matching birth data upgrades only the
provenance method version and preserves `CONTEXT.md` and chart artifacts.
